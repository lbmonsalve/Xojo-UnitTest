...
Protected Class TestController
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (not TargetHasGUI and not TargetWeb and not TargetIOS) or  (TargetWeb) or  (TargetHasGUI)
		Sub ExportTestResults(filePath As String)
		  ...
		  #If TargetWin32 Then
		    Dim kEOL As String= Chr(13) + Chr(10)
		  #Else
		    Dim kEOL As String= Chr(10)
		  #Endif
		  
		  Const frmt = "########0"
		  
		  #if RBVersion> 2015.01 // new framework
		    Dim testId As String= Xojo.Core.Date.Now.ToText + "." + Xojo.Core.Date.Now.Nanosecond.ToText
		  #else
		    Dim now As New Date
		    Dim testId As String= now.SQLDateTime + "." + Str(Microseconds, frmt)
		  #endif
		  ...
		  If f <> Nil Then
		    ...
		    """ tests=""" + Str(RunTestCount, frmt) + _
		    """ failures=""" + Str(FailedCount, frmt) + _
		    """ time=""" + Str(Duration, frmt) + """>" + kEOL
		    
		    For Each tg As TestGroup In mTestGroups
		      stream.Write "  <testsuite errors=""0"" skipped=""" + Str(tg.SkippedTestCount, frmt) + _
		      """ tests=""" + Str(tg.TestCount, frmt) + _
		      """ time=""" + Str(tg.Duration, frmt) + _
		      """ failures=""" + Str(tg.FailedTestCount, frmt) + _
		      """ name=""com.atlassian.bamboo.labels." + tg.Name + """>" + kEOL
		      
		      For Each tr As TestResult In tg.Results
		        stream.Write "    <testcase name=""" + tr.TestName + """ time=""" + Str(tr.Duration, frmt) + _
		        """ duration= """ + Str(tr.Duration, frmt) + """>" + kEOL // "time" is right, but "duration" is maintained for backwards compatibility
	...
	#tag Method, Flags = &h21, CompatibilityFlags = (TargetConsole and (Target32Bit or Target64Bit)) or  (TargetWeb and (Target32Bit or Target64Bit)) or  (TargetDesktop and (Target32Bit or Target64Bit))
		Private Function SimplePatternToRegExPattern(pattern As String) As String
		  Dim kPlaceholder As String = Chr(1)
	...
	#tag Constant, Name = XojoUnitVersion, Type = , Dynamic = False, Default = \"6.5", Scope = Public
	#tag EndConstant
